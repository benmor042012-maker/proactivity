...
'g.s':           ('כתוב מטרה והמערכת תפרק אותה לצעדים. אפשר להוסיף צעדים משלך.',
...
'w.s':           ('סמן כל יום שבו ביצעת. היעד הוא כמה ימים בשבוע, לא כל יום.',
                  ...
                  'ضع علامة على كل يوم نفّذته. الهدف عدد أيام في الأسبوع، لا كل يوم.'),
...
'w.empty':       ('אין משימות שבועיות. הוסף אחת למטה.', 'No weekly missions yet. Add one below.', 'Aucune mission hebdo. Ajoutes-en une.', 'Недельных задач нет. Добавь одну ниже.', 'لا مهام أسبوعية. أضف واحدة بالأسفل.'),
...
'w.rev.btn':     ('בדוק שבוע', 'Review week', 'Faire le bilan', 'Подвести итоги', 'راجع الأسبوع'),
